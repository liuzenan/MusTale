...
- (id)draggableViewWithImageURL:(NSURL *)imageURL
{
    ...
    view.layer.cornerRadius = 33.0f;
    view.backgroundColor = [UIColor whiteColor];
    self.songCover = [[UIImageView alloc] initWithFrame:CGRectInset(view.bounds, 4, 4)];
    self.songCover.backgroundColor = [UIColor lightGrayColor];
    self.songCover.layer.masksToBounds = YES;
    self.songCover.layer.cornerRadius = view.bounds.size.width / 2.0f - 4.0f;
    if (imageURL) {
        [self.songCover setImageWithURL:imageURL];
    }
    self.songCover.layer.shouldRasterize = view.layer.shouldRasterize = YES;
    self.songCover.layer.rasterizationScale = view.layer.rasterizationScale = [[UIScreen mainScreen] scale];
    
    self.songCover.center = CGPointMake(CGRectGetMidX(view.bounds), CGRectGetMidY(view.bounds));
    [view addSubview:self.songCover];
...
- (void)changeSongCover:(NSURL *)imageURL
{
    ...
    //[self.songCover setNeedsDisplay];
}
